...
def get_histo_surf(pictures):
    ...
    for key in pictures.keys():
        ...
        category.append(pictures[key]['category'])
        surf_instance = cv2.xfeatures2d.SURF_create(50000)

        #kp, des = surf_instance.detectAndCompute(pictures[key]['picture'],None)
        des = surf_instance.detect(pictures[key]['picture'])
...
def get_opencv_surf_model(train_pic, test_pic, neighbors):
    
    neighbors_instance = KNeighborsClassifier(n_neighbors = neighbors)
    
    histo_train, categories_train = get_histo_surf(train_pic)
    histo_test, categories_test = get_histo_surf(test_pic)
    print("Model processing")
    neighbors_instance.fit(histo_train, categories_train)
    score = round(100 * neighbors_instance.score(histo_test, categories_test), 1)
    print("Score on testing dataset  :", score, "%")
